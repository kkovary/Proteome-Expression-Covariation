...
allGroups <- allGroups %>% mutate(interval = cut_number(normVar, n = 10)) %>%
  mutate(interval = ifelse(interval == levels(interval)[1],
                           "low",
                           ifelse(interval == levels(interval)[10],
                                  "high",
                                  "mid")))

ggplot(allGroups, aes(x = normVar)) + geom_histogram(bins = 100, aes(fill = interval)) + theme_minimal() +
  #scale_fill_viridis_d()
  scale_fill_brewer(palette = "PiYG")

ggplot(allGroups, aes(x = medCors, y = medCV, color = interval)) + 
  geom_point() + 
  theme_minimal()
