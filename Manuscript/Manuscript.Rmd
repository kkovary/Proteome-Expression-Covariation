...
```{r figure_1, echo=FALSE, fig.height=8, fig.width=6, message=FALSE, warning=FALSE, fig.cap=fig_legend_1}
...
interesting_groups_low <- c("03010","00030","GO:0046496","00071","GO:0006096","GO:0015934","GO:0019843","00051")
...
p2 <- ggplot(filter(allGroups, n > 5), aes(x = medCors, y = medCV, color = interval)) + 
  geom_point(data = filter(allGroups, interval == "mid", n > 5),
...
(p1 + p3 + p4) / p2 + plot_layout(heights = c(1,5)) +
  plot_annotation(tag_levels = "A") & 
  theme(legend.position = "none")
...
glyc <- filter(allGroups, accession == '00010')$Uniprot %>% unlist()
...
row_anno <- data.frame(cv = unlist(filter(allGroups, accession == "00010") %>% pull(cv)))
...
p4 <- p1/p2 + plot_layout(heights = c(2,1))
...
cowplot::ggdraw() + cowplot::draw_image(magick::image_read_pdf("../images/preview.pdf", density = 600))
