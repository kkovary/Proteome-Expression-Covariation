...
protImpute <- pepTidy %>%
  ...
  #filter(n() >= 10) %>% # comment out if using KNN
  filter(n() > 5) %>%
  group_by(cell) %>%
  mutate(abundance = abundance / mean(abundance, na.rm = T)) %>%
  ungroup() %>%
  ...
  #spread(cell, abundance) %>%
  spread(Uniprot, abundance) %>%
  kNN(data = ., k = 3, impNA = TRUE) %>% as_tibble() %>%
  #dplyr::select(Uniprot:e_131_80)
  dplyr::select(cell:UBE2I_HUMAN) %>%
  pivot_longer(2:ncol(.), names_to = "Uniprot", values_to = "abundance") %>%
  spread(cell, abundance)
...
protVar <- protImpute %>% gather(key = 'cell', value = 'abundance', 2:ncol(protImpute)) %>%
  ...
  summarise(var = var(abundance, na.rm = TRUE),
...
allGroups <- allGroups %>% mutate(interval = cut_number(normVar, n = 7)) %>%
  mutate(int_range = interval,
         interval = ifelse(interval == levels(interval)[1],
                           "low",
...
* To understand the total variance of a protein module such as a metabolic pathway or protein complex, a single cell approach capable of measuring the varaince and correlation of proteins is necessary.
* For core housekeeping modules such as metabolic pathways and heteromeric complexes like the ribosome, individual cells enforce a balance between protein expression variance and co-expression variance.
* For certain modules increasing co-expression and reducing expression variance may allow for better control and more efficient opperation.

# Introduction

The coordinated expression of proteins is vital for cellular function, from maintaining a dynamic steady state to differentiation of cells into specialized types and tissues. This is especailly critical for heteromeric protein complexes and metabolic and signaling pathways where many proteins are required to work in coordination in order for the system to function. For prokaryotes (and in rare instances eukaryotes), this is accomplished in a rather uncomplicated fashion through the use of opperons that can be co-translated. Eukaryotes for the most part rely on transcriptional activators and repressors to coordinate the expression of these genes that may exist in clusters or scattered throughout the genome. The bursty nature of transcriptional dynamics, which is less of an issue in the case of opperons, leads to stochastic production of mRNA of each component of the pathway. There exist buffering mechanisms such as bottlenecking in post-processing of nacient mRNA and nuclear export. There also exists evidence of co-translation through specialized ribosomes in eukaryotes that can preferentially translate members of the same pathway.

This coordinated regulation of protein abundance is subject to stochastic expression levels, and modules (pathways, complexes, etc.) have varying tolerances of noise for proper function ranging from precise stoichiometric coordination, to high levels of noise that can result in heterogeneous responses of homogeneous cell populations to identical stimuli `r citep("10.1073/pnas.1615660114")`. The ability to regulate coordinated expression has been demonstrated to occur via transcription (transcription factors, chromatin regulation), translation (specialized ribosomes, mRNA structure/modifications), and degradation (E3 ligases), all of which are subject to stochastic expression and interactions. This noise has been shown to be utilized by cells for bet hedging strategies as well as tissue size maintenance (more concrete examples to come). Additionally, the combination of variability and coordinated expression (correlation) of proteins can lead to increased population level control in binary decisions, and a decreased ability to execute analog signaling `r citep(c("doi.org/10.15252/msb.20177997", "10.1073/pnas.1615660114"))`.

There have been numerous studies, both targeted and unbiased, of coordinated protein expression or expression variation independently that have shown the important impacts of these parameters on cellular function. However, both of these parameters (variation and correlation) act together to determine the total variance of a system. Additionally, the nature of these two sources of variance can have very different impacts on the behavior or function of different systems. For example, high amounts of variance can decrease the output of a metabolic pathway but can increase the population level control of a binary signalling pathway. On the other hand, high levels of correlated expression, while technically increasing the variance of a system, can increase the output of metabolic pathways. We wanted to study these two sources of variation to identify instances where there may be a trade off between them or if they're used in coordination with one another.

To systematically assess these properties of single cells at a large scale, we conducted shotgun proteomics analyses on *Xenopus laevis* eggs during their first cell cycle. We have found this model to be ideal at this nascent stage of the single cell proteomics field because of their large size and tractability. An advantage of this approach is that we can get around much of the signal to noise issues that accompany studying single cells caused by low sample protein levels. Additionally, at this stage of development transcription is restricted, allowing for insights into non-transcriptional control of protein expression. By utilizing isobaric tagging and shotgun mass spectrometry we were able to measure >1000 proteins across 25 single cells in 5 mass spectrometry runs. 

With this data set we have been able to, for the first time, gain insights into the relationship between protein expression variance, coordinated protein expression, and total variance within modules at a proteomic scale in single cells. We identified classes of proteins that include heteromeric complexes and metabolic pathways that are expressed in such a way that the increase in total variance of the system caused by high coordinated expression of proteins is offset by decreased variation, allowing for modules that rely on stoichiometric expression and low level of variance to maintain a low level of total system variance. By counter balancing coordinated expression (correlation) of proteins in a module with low expression variance, cells are able to decrease the total variance of a given complex or pathway. This elegant balancing act may allow for finer control of metabolic throughput and controlling the number of potentially formed complexes through stoichiometric control.

# Results

## Single cell proteomics reveals global protein expression variability and coordinated expression between protein pairs.

The specific requirements for coordinated protein expression of pathways or complexes can vary, with some requiring strict stoichiometric regulation and others having much more relaxed requirements. Strict stoichiometry scenarios require coordinated expression of proteins (Figure 1A, top), and others can have uncoordinated expression (Figure 1A bottom). At the single cell level, coordinated expression will result in a high correlation coefficient, whereas un-coordinated expression will result in a low correlation coefficient (Figure 1B). Even when the expression variation (standard deviation or coefficient of variation) and abundances are identical between these two scenarios, the total variance of a pathway or complex can be significantly different due to the variance sum law: $var\left(\sum_{i=1}^n X_{i}\right) = \sum_{i=1}^{n} var(X_{i}) + 2\sum_{i,j:i<j}^{n} cov(X_{i},X_{j})$ (Figure 1C). The nature of these two sources of variation that contribute to the variance sum law can have very different impacts on the behavior of systems. This means that for a given pathway with fixed total variance, the variance and correlation of the members of that system can be changed and the behavior can be impacted (needs big time re-wording, and first figure should show this...). Our previous work has shown that this effect is important in single cell pathway activation dynamics `r citep("doi.org/10.15252/msb.20177997")`.
...
```{r figure_1, echo=FALSE, fig.height=9, fig.width=8.5, message=FALSE, warning=FALSE, fig.cap=fig_legend_1}
...
cowplot::ggdraw() + cowplot::draw_image(magick::image_read_pdf("../images/Fig_1_new.pdf"))

fig_legend_1 <- c("Single cell approach to study expression variation and correlation at a proteomic scale: A) Simulated expression of two proteins over time in a single cell with means and standard deviations of 100 and 30 respectively and correlations of 0 and 0.8. B) Pairwise plots of the two proteins in the single cells between uncorrelated and correlated scenarios. C) The total variance between the uncorrelated and correlated scenarios from A and B. D) To measure protein expression variance and coordinated expression, 25 eggs at 5 time points during the first cell cycle were analyzed with multiplexed shotgun mass spectrometry. E) Proteins were grouped by KEGG IDs and GO terms. The expression variation and pairwise correlation coefficients were calculated for all proteins within each group (4 of 1225 shown) which were then used to calcuate the normalized total variance of each group.")

```

\newpage

# New Figure 1 sub-plots
```{r eval=FALSE, include=FALSE}
#hm_groups <- c("03010","00030","GO:0046496","00071","GO:0006096")
hm_groups <- c("GO:0015934", "04012", "GO:0007264", "GO:0010608", "GO:0031329", "GO:0000786", "00030")
#hm_groups <- allGroups$accession

rm(prot_hm)
for(i in seq_along(hm_groups)){
  if(!exists("prot_hm")){
    temp_prots <- allGroups %>% 
      filter(accession == hm_groups[i]) %>% 
      pull(Uniprot) %>% unlist()
    
    prot_hm <- protImpute %>% 
      filter(Uniprot %in% temp_prots) %>% 
      select(-Uniprot) %>%
      as.matrix()
    
    Group <- rep(filter(allGroups, 
                        accession == hm_groups[i])$name,
                 times = length(temp_prots))
    
    mat_labels <- temp_prots
  } else{
    temp_prots <- allGroups %>% 
      filter(accession == hm_groups[i]) %>% 
      pull(Uniprot) %>% unlist()
    
    temp_prot_hm <- protImpute %>% 
      filter(Uniprot %in% temp_prots) %>% 
      select(-Uniprot) %>%
      as.matrix()
    
    prot_hm <- rbind(prot_hm, temp_prot_hm)
    
    Group <- append(Group,
                    rep(filter(allGroups, 
                        accession == hm_groups[i])$name,
                 times = length(temp_prots)),
                 after = length(Group))
    
    mat_labels <- append(mat_labels, temp_prots, after = length(mat_labels))
  }
}
mat_labels <- mapIds(org.Xl.eg.db, 
       keys = mat_labels, 
       keytype = 'UNIPROT', 
       column = 'SYMBOL')

rownames(prot_hm) = mat_labels

df <- tibble(run = rep(LETTERS[1:5], each = 5), 
  minutes = as.character(rep(seq(0,80,20), times = 5))) %>%
  unite(run,minutes, col = "x")

col <- c(brewer.pal(6,"Blues")[2:6],
         brewer.pal(6,"Greens")[2:6],
         brewer.pal(6,"Oranges")[2:6],
         brewer.pal(6,"Purples")[2:6],
         brewer.pal(6,"Reds")[2:6])
names(col) <- df$x
...
# column_ha = HeatmapAnnotation(
#   run = rep(LETTERS[1:5], each = 5), 
#   minutes = as.character(rep(seq(0,80,20), times = 5)),
#   col = list(
#     run = c("A" = "#e41a1c", "B" = "#377eb8", "C" = "#4daf4a", "D" = "#984ea3", "E" = "#ff7f00"),
#     minutes = c("0" = "#ffffcc", "20" = "#a1dab4", "40" = "#41b6c4", "60" = "#2c7fb8", "80" = "#253494")
#   )
# )
...
Heatmap(
  ...
  name = "log2()",
  #border = "black",
  col = colorRamp2(seq(-3,3,0.01), rev(colorRampPalette(brewer.pal(11, 'RdBu'))(601))),
...
df <- prot_hm %>% as_tibble %>% cbind(Group, .) %>% as_tibble() %>%
  mutate(Prot = as.factor(rownames(prot_hm))) %>%
  group_by(Group) %>% 
  #mutate(Prot = 1:n()) %>% 
  select(Group, Prot, everything()) %>%
  ungroup() %>%
  pivot_longer(3:ncol(.), names_to = "cell", values_to = "abundance")
  
ggplot(df, aes(x = log2(abundance), y = as.factor(Prot), fill = Group)) + 
  ggridges::geom_density_ridges(scale = 2, size = 0.1) +
...
df %>% group_by(Group, Prot) %>%
  summarise(mean = mean(abundance),
            sd = sd(abundance),
            cv = sd / mean) %>%
  #mutate(order = rank(sd)) %>%
  ggplot(., aes(x = fct_reorder(Prot, rank(cv)), y = cv, fill = Group)) +
  geom_bar(stat = "identity") + 
  geom_hline(yintercept = median(allGroups$medCV), linetype = "dashed") +
  facet_wrap(~Group, scales = "free_y", ncol = 1) +
  theme_minimal() +
  theme(legend.position = "none",
         axis.title.y=element_blank(),
...
test_prots <- allGroups %>% filter(accession == hm_groups[7]) %>% pull(Uniprot) %>% unlist()
test_cor_hm <- cor[test_prots, test_prots]
test_prots <- mapIds(org.Xl.eg.db, 
       keys = test_prots, 
       keytype = 'UNIPROT', 
       column = 'SYMBOL')
rownames(test_cor_hm) = test_prots
colnames(test_cor_hm) = test_prots
...
allGroups %>% filter(accession %in% hm_groups) %>% 
  ggplot(., aes(x = fct_reorder(name, -medCV), y = medCV, fill = name)) + 
  geom_bar(stat = "identity") +
  geom_hline(yintercept = median(allGroups$medCV), linetype = "dashed") +
  coord_flip() +
  theme_minimal() +
...
allGroups %>% filter(accession %in% hm_groups) %>% 
  ggplot(., aes(x = fct_reorder(name, medCors), y = medCors, fill = name)) + 
  geom_bar(stat = "identity") +
  geom_hline(yintercept = median(allGroups$medCors), linetype = "dashed") +
  coord_flip() +
  theme_minimal() +
  theme(legend.position = "none")

# Histogram w/ variance groups of interest
allGroups %>% 
  ggplot(., aes(x = medCV)) +
  geom_histogram(bins = 50) +
  geom_histogram(data = filter(allGroups,
                               accession %in% hm_groups),
                 aes(x = medCV, fill = name),
                 bins = 50) +
  theme_minimal() +
  theme(legend.position = "bottom")

allGroups %>% 
  ggplot(., aes(x = medCors)) +
  geom_histogram(bins = 50) +
  geom_histogram(data = filter(allGroups,
                               accession %in% hm_groups),
                 aes(x = medCors, fill = name),
                 bins = 50) +
  theme_minimal() +
  theme(legend.position = "bottom")
```
...
Goal: do a positive and negatvie correlation Gini analysis. Since this analysis only works with positive values, and because 

```{r}
library(diptest)

for(i in seq_along(hm_groups)){
  test_prots <- allGroups %>% 
    filter(accession == hm_groups[i]) %>% 
    pull(Uniprot) %>% 
    unlist()
  ...
  print(dip.test(test_cor_hm[lower.tri(test_cor_hm)])$p.value)
}

test_prots <- allGroups %>% 
  filter(accession == hm_groups[8]) %>% pull(Uniprot) %>% unlist()

test_cor_hm <- cor[test_prots, test_prots]

Heatmap(test_cor_hm,
        col = colorRamp2(seq(-1,1,0.01), 
                         rev(colorRampPalette(brewer.pal(11, 'RdBu'))(201))),
        show_row_names = FALSE,
        show_column_names = FALSE,
        show_heatmap_legend = FALSE,
        show_column_dend = FALSE,
...
ggplot(allGroups, aes(x = medCV, y = sdCV)) + 
  geom_point(alpha = 0.5, color = "grey") +
  geom_point(data = filter(allGroups, accession %in% hm_groups),
             color = "orange",
             size = 2) +
  geom_text_repel(data = filter(allGroups, accession %in% hm_groups),
                  aes(label = name),
                  force = 10,
                  box.padding = 1) +
  theme_minimal()
...
```{r figure_2, echo=FALSE, fig.height=9, fig.width=8.5, message=FALSE, warning=FALSE, eval=TRUE, fig.cap=fig_legend_2}
...
interesting_groups_low <- c("03010","00030","GO:0046496","00071",
                            ...
                            "00051","GO:0042555","04012","GO:0006098")
...
p2 <- ggplot(filter(allGroups, n > 3), aes(x = medCors, y = medCV, color = interval, shape = type)) +
  geom_point(data = filter(allGroups, interval == "mid", n > 3),
             size = 2, alpha = 0.5, stroke = 0) +
  geom_point(data = filter(allGroups, interval != "mid", n > 3),
             size = 3, alpha = 0.75, stroke = 0) +
  geom_label_repel(
    data = filter(allGroups, accession %in% interesting_groups),
    #data = filter(allGroups, medCors > 0.15, interval != "mid", n > 4),
    aes(label = name),
    colour = 'black',
    size = 1.5,
    force = 5,
...
left <- p5 + p6 + p7 + p8 + plot_layout(nrow = 1)

right <- p9 + p10 + p11 + p12 + plot_layout(nrow = 1)

bottom <- wrap_plots(left, right) + 
  plot_annotation(tag_levels = "A") + 
  plot_layout(nrow = 2)
...
mid <- p3 + plot_spacer() + p2 + (p4 + coord_flip()) +
  plot_layout(widths = c(4,1),
              heights = c(1,4))
p1 / mid / bottom +
  plot_layout(heights = c(0.5,4,3)) +
  plot_annotation(tag_levels = "A")

fig_legend_2 <- c("Tradeoff between expression variation and co-expression in low variation modules: A) Histogram of the normalized variation values across all of the GO terms and KEGG pathways. The bottom and top ~15% are colored blue and red respectively. B) Distributions of the mean correlation coefficient across all groups of proteins. Both the top and bottom 15% of normalized variation groups had peaks of high levels of mean correlation coefficients. C) Pairwise plot between the mean correlation and mean coefficient of variation within each GO term and KEGG pathway with color coding from A. Interestingly, both the top and bottom 10% of normalized variation have groups of proteins with high levels of correlation, despite high correlation adding to total variance. In the lowest 10% of normalized variation groups there appears to be a negative correlation between mean correlation and variation. D) Distributions of the mean coefficient of variation across all groups of proteins. The top and bottom ~15% of normalized variation groups were largely separated. E:L) Density plots of correlation coefficients select protein groups color coded by low (blue) and high (red) normalized protein variance, with the distribution of all measured correlation coefficients colored in grey (rug plot to show the measured correlations coefficients shown on plot). Below each density plot is a box plot.")

```


```{r figure_3, echo=FALSE, fig.height=4, fig.width=7, message=FALSE, warning=FALSE, eval=FALSE, fig.cap=fig_legend_3}
...
```{r}
pca = prcomp(log2(protImpute[,2:ncol(protImpute)]))

pcaDF = tibble(cell = rownames(pca$rotation), PC1 = pca$rotation[,1], PC2 = pca$rotation[,2])
pcaDF = pcaDF %>% separate(cell, into = c('run','label',"time"), remove = F)
ggplot(pcaDF, aes(x = PC1, y = PC2, colour = time)) + geom_point(size = 5) +
  theme_minimal() +
  theme(legend.position = "bottom")
```
...
Single eggs were lysed mechanically by pipetting the egg in 100$\mu$L of lysis buffer (100 mM NaCl, 25 mM Tris pH 8.2, Complete EDTA- free protease inhibitor cocktail (Sigma). The lysate was then placed in a 400 uL natural polyethylene micro-centrifuge tube (E&K
